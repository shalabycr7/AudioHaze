...
class AudioProcessing(object):
    ...
    def _set_stretch(self, factor, window_size, h):
        ...
        han_window = np.hanning(window_size)
        ...
        for i in np.arange(0, len(self.audio_data) - (window_size + h), h * factor):
            # Two potentially overlapping arrays
            a1 = self.audio_data[int(i): int(i + window_size)]
            a2 = self.audio_data[int(i + h): int(i + window_size + h)]

            # The spectra of these arrays
            s1 = np.fft.fft(han_window * a1)
            s2 = np.fft.fft(han_window * a2)

            # Rephrase all frequencies
            phase = (phase + np.angle(s2 / s1)) % 2 * np.pi

            a2_rephrased = np.fft.ifft(np.abs(s2) * np.exp(1j * phase))
            i2 = int(i / factor)
            result[i2: i2 + window_size] += han_window * a2_rephrased.real
